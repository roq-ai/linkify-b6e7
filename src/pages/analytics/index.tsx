...
import { getAnalytics, deleteAnalyticsById } from 'apiSdk/analytics';
import { AnalyticsInterface } from 'interfaces/analytics';

type ColumnType = ColumnDef<AnalyticsInterface, unknown>;

interface AnalyticsListPageProps {
...
export function AnalyticsListPage(props: AnalyticsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AnalyticsInterface>>(
  ...
  const handleView = (row: AnalyticsInterface) => {
    if (hasAccess('analytics', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/analytics/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/analytics/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('analytics', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/analytics/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="analytics" />
        </Flex>

        {hasAccess('analytics', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/analytics/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

...
import { getSubscriptions } from 'apiSdk/subscriptions';
import { SubscriptionInterface } from 'interfaces/subscription';
import { getThemeCustomizations } from 'apiSdk/theme-customizations';
import { ThemeCustomizationInterface } from 'interfaces/theme-customization';

import { LinkClicksListPage as ShowCaseList } from 'pages/link-clicks';

function DashboardPage() {
  const { session } = useSession();

  const profileStat = useSWR<PaginatedInterface<ProfileInterface>>(
    () => `/profiles`,
    () => getProfiles({ limit: 1, offset: 0 }),
  );

  const subscriptionStat = useSWR<PaginatedInterface<SubscriptionInterface>>(
    () => `/subscriptions`,
    () => getSubscriptions({ limit: 1, offset: 0 }),
  );

  const themeCustomizationStat = useSWR<PaginatedInterface<ThemeCustomizationInterface>>(
    () => `/theme-customizations`,
    () => getThemeCustomizations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Profiles',
      result: profileStat?.data?.totalCount,
      isLoading: profileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Subscriptions',
      result: subscriptionStat?.data?.totalCount,
      isLoading: subscriptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Theme Customizations',
      result: themeCustomizationStat?.data?.totalCount,
      isLoading: themeCustomizationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`link-clicks`} passHref legacyBehavior>
                  <Button

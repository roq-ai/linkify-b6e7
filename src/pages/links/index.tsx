...
import { getLinks, deleteLinkById } from 'apiSdk/links';
import { LinkInterface } from 'interfaces/link';

type ColumnType = ColumnDef<LinkInterface, unknown>;
...
export function LinkListPage(props: LinkListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LinkInterface>>(
    () => `/links?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: LinkInterface) => {
    if (hasAccess('link', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/links/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/links/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('link', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/links/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('link', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="link" />
        </Flex>

        {hasAccess('link', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/links/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

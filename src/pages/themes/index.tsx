...
import { getThemes, deleteThemeById } from 'apiSdk/themes';
import { ThemeInterface } from 'interfaces/theme';

type ColumnType = ColumnDef<ThemeInterface, unknown>;

interface ThemeListPageProps {
...
export function ThemeListPage(props: ThemeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ThemeInterface>>(
    () => `/themes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ThemeInterface) => {
    if (hasAccess('theme', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/themes/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/themes/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('theme', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/themes/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="theme" />
        </Flex>

        {hasAccess('theme', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/themes/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
